import React from "react";
import logo from "../../assets/img/logo.svg";
import badge from "../../assets/img/Badge.svg";
import "../../assets/css_dashboard/mdb.min.css";
import "../../assets/css_dashboard/style.css";
import "../../assets/css_dashboard/slick.css";
import avatar from "../../assets/img/avatar-1.png";
import { Link } from "react-router-dom";
export default function Sidebar(props) {
  return (
    <div className="col-sm-2 side-menu">
      <div className="row m-0 p-0 position-relative desktop">
        <nav className="navbar navbar-expand-lg m-0 desktop">
          <div
            className="navbar-nav collapse navbar-collapse"
            id="navbarSupportedContent"
          >
            <Link to={"/dashboard"} className="navbar-brand mt-0 mt-5 py-3">
              <img src={logo} height="" alt="Main Compaign" loading="lazy" />
            </Link>
          </div>
        </nav>

        <div className="links mt-sm-5">
          <Link to={"/dashboard"} className="d-flex align-items-center">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_96_408)">
                <path
                  d="M15.8333 4.16667V5.83333H12.5V4.16667H15.8333ZM7.5 4.16667V9.16667H4.16667V4.16667H7.5ZM15.8333 10.8333V15.8333H12.5V10.8333H15.8333ZM7.5 14.1667V15.8333H4.16667V14.1667H7.5ZM17.5 2.5H10.8333V7.5H17.5V2.5ZM9.16667 2.5H2.5V10.8333H9.16667V2.5ZM17.5 9.16667H10.8333V17.5H17.5V9.16667ZM9.16667 12.5H2.5V17.5H9.16667V12.5Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_96_408">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>
            <span className="ms-3">Dashboard</span>
          </Link>
          <Link to={"/dashboard"} className="d-flex align-items-center">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_99_194)">
                <path
                  d="M15 9.16675C15 9.72508 15 10.2751 15 10.8334C16 10.8334 17.3 10.8334 18.3333 10.8334C18.3333 10.2751 18.3333 9.72508 18.3333 9.16675C17.3 9.16675 16 9.16675 15 9.16675Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M13.333 14.6749C14.133 15.2666 15.1747 16.0499 15.9997 16.6666C16.333 16.2249 16.6663 15.7749 16.9997 15.3333C16.1747 14.7166 15.133 13.9333 14.333 13.3333C13.9997 13.7833 13.6663 14.2333 13.333 14.6749Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M16.9997 4.66659C16.6663 4.22492 16.333 3.77492 15.9997 3.33325C15.1747 3.94992 14.133 4.73325 13.333 5.33325C13.6663 5.77492 13.9997 6.22492 14.333 6.66659C15.133 6.06659 16.1747 5.29159 16.9997 4.66659Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M3.33366 7.5C2.41699 7.5 1.66699 8.25 1.66699 9.16667V10.8333C1.66699 11.75 2.41699 12.5 3.33366 12.5H4.16699V15.8333H5.83366V12.5H6.66699L10.8337 15V5L6.66699 7.5H3.33366ZM7.52533 8.925L9.16699 7.94167V12.0583L7.52533 11.075L7.12533 10.8333H6.66699H3.33366V9.16667H6.66699H7.12533L7.52533 8.925Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M12.917 9.99992C12.917 8.89159 12.4337 7.89159 11.667 7.20825V12.7833C12.4337 12.1083 12.917 11.1083 12.917 9.99992Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_99_194">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>

            <span className="ms-3">Campaign</span>
          </Link>
          <Link to={"/dashboard"} className="d-flex align-items-center">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_100_202)">
                <path
                  d="M19.1667 9.17508L15 9.16675C14.5417 9.16675 14.1667 9.54175 14.1667 10.0001V17.5001C14.1667 17.9584 14.5417 18.3334 15 18.3334H19.1667C19.625 18.3334 20 17.9584 20 17.5001V10.0001C20 9.54175 19.625 9.17508 19.1667 9.17508ZM19.1667 16.6667H15V10.8334H19.1667V16.6667ZM16.6667 1.66675H1.66667C0.741667 1.66675 0 2.40841 0 3.33341V13.3334C0 14.2501 0.741667 15.0001 1.66667 15.0001H7.5V16.6667H5.83333V18.3334H12.5V16.6667H10.8333V15.0001H12.5V13.3334H1.66667V3.33341H16.6667V7.50008H18.3333V3.33341C18.3333 2.40841 17.5833 1.66675 16.6667 1.66675ZM9.975 7.50008L9.16667 5.00008L8.35833 7.50008H5.83333L7.89167 8.96675L7.10833 11.3917L9.16667 9.89175L11.225 11.3917L10.4417 8.96675L12.5 7.50008H9.975Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_100_202">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>

            <span className="ms-3">Ads Manager</span>
          </Link>
          <Link to={"/dashboard"} className="d-flex align-items-center">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_194_302)">
                <path
                  d="M3.33366 5.00008H16.667V6.66675H3.33366V5.00008ZM5.00033 1.66675H15.0003V3.33341H5.00033V1.66675ZM16.667 8.33341H3.33366C2.41699 8.33341 1.66699 9.08341 1.66699 10.0001V16.6667C1.66699 17.5834 2.41699 18.3334 3.33366 18.3334H16.667C17.5837 18.3334 18.3337 17.5834 18.3337 16.6667V10.0001C18.3337 9.08341 17.5837 8.33341 16.667 8.33341ZM16.667 16.6667H3.33366V10.0001H16.667V16.6667ZM8.33366 10.6084V16.0501L13.3337 13.3334L8.33366 10.6084Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_194_302">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>
            <span className="ms-3">Ad Sets</span>
          </Link>
          <Link to={"/dashboard"} className="d-flex align-items-center">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_194_305)">
                <path
                  d="M17.842 9.65008L10.342 2.15008C10.042 1.85008 9.62533 1.66675 9.16699 1.66675H3.33366C2.41699 1.66675 1.66699 2.41675 1.66699 3.33341V9.16675C1.66699 9.62508 1.85033 10.0417 2.15866 10.3501L9.65866 17.8501C9.95866 18.1501 10.3753 18.3334 10.8337 18.3334C11.292 18.3334 11.7087 18.1501 12.0087 17.8417L17.842 12.0084C18.1503 11.7084 18.3337 11.2917 18.3337 10.8334C18.3337 10.3751 18.142 9.95008 17.842 9.65008ZM10.8337 16.6751L3.33366 9.16675V3.33341H9.16699V3.32508L16.667 10.8251L10.8337 16.6751Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M5.41699 6.66675C6.10735 6.66675 6.66699 6.1071 6.66699 5.41675C6.66699 4.72639 6.10735 4.16675 5.41699 4.16675C4.72664 4.16675 4.16699 4.72639 4.16699 5.41675C4.16699 6.1071 4.72664 6.66675 5.41699 6.66675Z"
                  fill="#7D889E"
                ></path>
                <path
                  d="M7.41699 10.4583C7.41699 10.9333 7.60866 11.35 7.91699 11.6667L10.8337 14.5833L13.7503 11.6667C14.0587 11.3583 14.2503 10.925 14.2503 10.4583C14.2503 9.51667 13.4837 8.75 12.542 8.75C12.067 8.75 11.642 8.94167 11.3337 9.25L10.8337 9.75L10.3337 9.25833C10.0253 8.94167 9.59199 8.75 9.12533 8.75C8.18366 8.75 7.41699 9.51667 7.41699 10.4583Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_194_305">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>

            <span className="ms-3">Ads</span>
          </Link>
          <Link className="d-flex align-items-center" to={"/dashboard"}>
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_195_310)">
                <path
                  d="M12.3503 9.16658H18.292C17.9003 5.23325 14.767 2.09992 10.8337 1.70825V7.64992C11.542 7.90825 12.092 8.45825 12.3503 9.16658ZM12.5003 3.81659C14.167 4.49992 15.5003 5.83325 16.1837 7.49992H13.3253C13.092 7.19159 12.8087 6.90825 12.5003 6.67492V3.81659ZM1.66699 9.99992C1.66699 14.3249 4.95866 17.8749 9.16699 18.2916V12.3499C8.20033 11.9999 7.50033 11.0833 7.50033 9.99992C7.50033 8.91658 8.20033 7.99992 9.16699 7.64992V1.70825C4.95866 2.12492 1.66699 5.67492 1.66699 9.99992ZM7.50033 3.81659V6.68325C6.47533 7.44992 5.83366 8.67492 5.83366 9.99992C5.83366 11.3249 6.47533 12.5499 7.50033 13.3249V16.1916C5.03366 15.1999 3.33366 12.7916 3.33366 9.99992C3.33366 7.20825 5.03366 4.79992 7.50033 3.81659ZM10.8337 12.3499V18.2916C14.767 17.8999 17.9003 14.7666 18.292 10.8333H12.3503C12.092 11.5416 11.542 12.0916 10.8337 12.3499ZM12.5003 13.3249C12.8087 13.0916 13.092 12.8166 13.3253 12.4999H16.1837C15.5003 14.1666 14.167 15.4999 12.5003 16.1833V13.3249Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_195_310">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>

            <span className="ms-3">Analytics</span>
          </Link>
          <Link className="d-flex align-items-center" to={"/dashboard"}>
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_193_296)">
                <path
                  d="M13.3337 9.16667V2.5H6.66699V7.5H1.66699V17.5H18.3337V9.16667H13.3337ZM8.33366 4.16667H11.667V15.8333H8.33366V4.16667ZM3.33366 9.16667H6.66699V15.8333H3.33366V9.16667ZM16.667 15.8333H13.3337V10.8333H16.667V15.8333Z"
                  fill="#7D889E"
                ></path>
              </g>
              <defs>
                <clipPath id="clip0_193_296">
                  <rect width="20" height="20" fill="white"></rect>
                </clipPath>
              </defs>
            </svg>
            <span className="ms-3">Leads</span>
          </Link>
        </div>
        <h5 className="font-16 color-blue mt-5 pt-3">MORE</h5>
        <div className="links">
          <Link className="d-flex align-items-center" to={"/dashboard"}>
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M7.84059 0C8.61395 0 9.38732 0 10.1607 0C10.4239 0.0922761 10.5561 0.286496 10.6076 0.553657C10.6484 0.765892 10.7003 0.97637 10.7609 1.18377C10.7767 1.23826 10.8352 1.30768 10.8866 1.32087C11.6692 1.51728 12.4039 1.82575 13.0977 2.23704C13.1443 2.26472 13.2339 2.26121 13.2831 2.2344C13.4888 2.12367 13.6856 1.99624 13.8878 1.87892C14.1839 1.70711 14.4045 1.73347 14.6479 1.97471C15.1093 2.43214 15.5685 2.89176 16.0264 3.3527C16.2689 3.59701 16.2957 3.81628 16.1239 4.11244C16.0066 4.31457 15.8792 4.51143 15.7684 4.71707C15.7416 4.76672 15.7372 4.85592 15.7645 4.90206C16.1758 5.59589 16.4842 6.33058 16.6811 7.11317C16.6938 7.16458 16.7624 7.2239 16.8164 7.23972C17.0238 7.30036 17.2343 7.35265 17.4461 7.39352C17.7133 7.44493 17.9088 7.57543 18.0006 7.83952C18.0006 8.61288 18.0006 9.38624 18.0006 10.1596C17.9097 10.4145 17.7251 10.5489 17.4628 10.5999C17.2448 10.6421 17.0291 10.6966 16.8164 10.7594C16.7624 10.7752 16.6938 10.8345 16.6811 10.8864C16.4842 11.669 16.1758 12.4037 15.7645 13.0971C15.7372 13.1432 15.7416 13.2328 15.7684 13.2821C15.8822 13.4925 16.0136 13.6942 16.1323 13.9021C16.2927 14.1828 16.2663 14.403 16.0387 14.6337C15.5738 15.1038 15.1062 15.5718 14.6356 16.0367C14.4063 16.2635 14.183 16.2889 13.9031 16.129C13.7001 16.013 13.5046 15.8834 13.2976 15.7753C13.2401 15.7449 13.1372 15.741 13.0832 15.7726C12.3977 16.1743 11.6749 16.4796 10.9037 16.6747C10.8444 16.6897 10.7776 16.7701 10.7583 16.8334C10.6963 17.0342 10.6475 17.2394 10.6076 17.4459C10.5561 17.7131 10.4239 17.9068 10.1607 17.9996C9.38732 17.9996 8.61395 17.9996 7.84059 17.9996C7.57826 17.9051 7.44644 17.7109 7.39547 17.4433C7.35636 17.2367 7.30671 17.0311 7.24431 16.8303C7.22498 16.7675 7.15775 16.6958 7.09623 16.6743C6.88751 16.5996 6.66737 16.5565 6.45909 16.4814C6.17611 16.3795 5.89972 16.2591 5.6119 16.1431C5.64135 16.1035 5.65013 16.0877 5.662 16.0754C6.44063 15.2963 7.21795 14.5164 8.00142 13.7426C8.04887 13.6955 8.14906 13.6692 8.21717 13.6802C8.69041 13.7571 9.16146 13.7408 9.63427 13.6788C10.8031 13.5259 11.7909 13.0215 12.5621 12.1317C13.5213 11.0244 13.8987 9.72854 13.6764 8.28113C13.4321 6.69222 12.5818 5.50493 11.1515 4.77815C9.70896 4.04521 8.23475 4.06323 6.79963 4.80803C5.58027 5.44078 4.7924 6.44571 4.42506 7.7758C4.24402 8.43096 4.23391 9.09315 4.33014 9.76018C4.34684 9.87531 4.3174 9.95484 4.23391 10.0379C3.49087 10.7774 2.75222 11.5222 2.01225 12.2653C1.97183 12.3061 1.92877 12.3444 1.87868 12.3918C1.85099 12.3386 1.82463 12.2995 1.80925 12.2565C1.65238 11.8228 1.48936 11.3908 1.34743 10.9523C1.30568 10.8236 1.2534 10.7581 1.12113 10.7344C0.937022 10.7014 0.757739 10.6438 0.574066 10.6096C0.302511 10.5586 0.0986234 10.4294 0.00195312 10.1596C0.00195312 9.38624 0.00195312 8.61288 0.00195312 7.83952C0.0986234 7.57016 0.30207 7.44141 0.573186 7.38956C0.779709 7.35002 0.984477 7.29948 1.18617 7.24016C1.24065 7.2239 1.31052 7.1659 1.32326 7.11493C1.51968 6.33234 1.82814 5.59765 2.23943 4.90382C2.26711 4.85724 2.2636 4.7676 2.23723 4.71839C2.1265 4.51274 1.99951 4.31589 1.88175 4.11376C1.70994 3.81848 1.73631 3.59657 1.97754 3.35358C2.43497 2.8922 2.89459 2.43302 3.35553 1.97559C3.59941 1.73347 3.81999 1.70667 4.11527 1.87804C4.3174 1.99536 4.51426 2.12279 4.7199 2.23352C4.76955 2.26033 4.8592 2.26472 4.90533 2.23748C5.59916 1.82619 6.33342 1.51772 7.116 1.32087C7.16742 1.30812 7.22674 1.23958 7.24256 1.18509C7.30363 0.978127 7.35636 0.76765 7.39679 0.555415C7.44644 0.287814 7.5787 0.0944732 7.84059 0Z"
                fill="#7D889E"
              ></path>
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M1.75764 18C1.64032 18 1.52344 18 1.40611 18C1.37931 17.9903 1.35338 17.9767 1.3257 17.9718C0.931546 17.9007 0.606825 17.7148 0.350648 17.4028C0.155111 17.1651 0.0694268 16.8839 0 16.5934C0 16.4761 0 16.3592 0 16.2419C0.0742603 15.733 0.380965 15.3701 0.732053 15.0216C2.29504 13.4701 3.84835 11.9093 5.40826 10.3547C5.4935 10.2699 5.5124 10.1991 5.4834 10.0809C5.41529 9.80365 5.33531 9.52199 5.32301 9.23944C5.26237 7.86102 5.78308 6.74535 6.92071 5.95441C7.84215 5.31331 8.86905 5.16875 9.95439 5.43196C10.1978 5.49084 10.3762 5.69604 10.3464 5.92585C10.327 6.07613 10.2488 6.24223 10.146 6.3534C9.74831 6.7827 9.31505 7.17949 8.92398 7.61407C8.60673 7.96691 8.62342 8.5298 8.91563 8.89583C9.32736 9.41213 10.0273 9.4486 10.5195 8.97053C10.9101 8.59088 11.2867 8.19716 11.6778 7.81839C12.0025 7.50377 12.4762 7.64087 12.5777 8.07545C12.699 8.59351 12.7258 9.11509 12.6295 9.64282C12.1862 12.0684 9.91924 13.123 7.92124 12.5232C7.80568 12.4884 7.73405 12.5043 7.64749 12.5908C6.09769 14.1468 4.54043 15.6957 2.9937 17.2552C2.63998 17.6115 2.26867 17.9156 1.75764 18Z"
                fill="#7D889E"
              ></path>
            </svg>

            <span className="ms-3">Setting</span>
          </Link>
          <Link className="d-flex align-items-center" to={"/dashboard"}>
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M9 0C4.032 0 0 4.032 0 9C0 13.968 4.032 18 9 18C13.968 18 18 13.968 18 9C18 4.032 13.968 0 9 0ZM15.714 6.408L13.212 7.443C12.753 6.219 11.79 5.247 10.557 4.797L11.592 2.295C13.482 3.015 14.985 4.518 15.714 6.408ZM9 11.7C7.506 11.7 6.3 10.494 6.3 9C6.3 7.506 7.506 6.3 9 6.3C10.494 6.3 11.7 7.506 11.7 9C11.7 10.494 10.494 11.7 9 11.7ZM6.417 2.286L7.47 4.788C6.228 5.238 5.247 6.219 4.788 7.461L2.286 6.417C3.015 4.518 4.518 3.015 6.417 2.286ZM2.286 11.583L4.788 10.548C5.247 11.79 6.219 12.762 7.461 13.212L6.408 15.714C4.518 14.985 3.015 13.482 2.286 11.583ZM11.592 15.714L10.557 13.212C11.79 12.753 12.762 11.781 13.212 10.539L15.714 11.592C14.985 13.482 13.482 14.985 11.592 15.714Z"
                fill="#7D889E"
              ></path>
            </svg>
            <span className="ms-3">Support</span>
          </Link>
        </div>
      </div>
      <div className="row m-0 p-0 position-relative mobile-menu-cont mobile">
        <header className="nav-top d-flex p-0">
          <div className="me-auto d-flex">
            <span
              className="hamburger material-icons pe-2 "
              id="ham"
              onClick={props.clickEvent}
            >
              <svg
                width="23"
                height="15"
                viewBox="0 0 23 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M0 15H22.5V12.5H0V15ZM0 0V2.5H22.5V0H0ZM0 8.75H15V6.25H0V8.75Z"
                  fill="#0A0A0D"
                ></path>
              </svg>
            </span>
            <Link className="navbar-brand mt-lg-0 ms-2" to={"/dashboard"}>
              <img
                src={logo}
                height="18"
                width="120"
                alt="Logo"
                loading="lazy"
              />
            </Link>
          </div>
          <div className="ms-auto d-flex align-items-center">
            <div className="grey-btn d-flex align-items-center justify-content-center">
              <Link
                to={"/dashboard"}
                className="d-flex align-items-center justify-content-center"
              >
                <svg
                  width="15"
                  height="15"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M13.8848 24C12.3848 24 10.8848 24 9.38475 24C9.35687 23.9871 9.33018 23.9692 9.30087 23.9622C8.4547 23.759 7.93513 23.1992 7.79561 22.3408C7.73429 21.9635 7.67141 21.5864 7.60709 21.2095C7.44563 20.2639 6.58427 19.7618 5.67959 20.0883C5.2836 20.2313 4.8933 20.3904 4.49557 20.5281C3.60645 20.8361 2.70352 20.5128 2.22871 19.7038C1.56525 18.5731 0.909733 17.4377 0.262463 16.2978C-0.208597 15.4682 -0.033825 14.5287 0.69233 13.9079C1.01281 13.6339 1.34541 13.374 1.66627 13.1004C2.38117 12.4908 2.38105 11.508 1.66652 10.8997C1.35729 10.6365 1.03819 10.3851 0.72821 10.1227C-0.0443885 9.46889 -0.21466 8.53665 0.285778 7.6616C0.921109 6.55077 1.56069 5.44226 2.20527 4.33674C2.71121 3.46907 3.61927 3.14809 4.56114 3.49413C4.94906 3.63665 5.33205 3.79311 5.7226 3.92781C6.59114 4.22735 7.44669 3.71791 7.60409 2.81367C7.67097 2.42937 7.73335 2.04426 7.79574 1.65921C7.93482 0.800852 8.45382 0.241281 9.30037 0.0375672C9.32975 0.0305038 9.35656 0.0128141 9.38463 0C10.8846 0 12.3846 0 13.8846 0C13.9126 0.0127516 13.9394 0.0305658 13.9688 0.0376292C14.8138 0.240155 15.3338 0.800727 15.4735 1.65915C15.5348 2.03651 15.5974 2.41362 15.6621 2.79042C15.8246 3.73635 16.6844 4.23735 17.5902 3.91112C17.9789 3.77111 18.3622 3.61559 18.7514 3.47713C19.6464 3.15878 20.5601 3.47863 21.0406 4.29617C21.7048 5.42632 22.3598 6.56196 23.0067 7.70211C23.4773 8.53152 23.3025 9.46971 22.5764 10.0925C22.2564 10.367 21.9234 10.6264 21.6026 10.9001C20.8872 11.5108 20.8874 12.4883 21.603 13.1002C21.9117 13.3641 22.2313 13.6149 22.5414 13.8773C23.3134 14.5303 23.4838 15.4639 22.9835 16.3388C22.3482 17.4496 21.7086 18.5582 21.0639 19.6635C20.5581 20.5304 19.6495 20.851 18.7072 20.5054C18.3192 20.3632 17.9362 20.207 17.5457 20.072C16.6772 19.772 15.8205 20.2823 15.665 21.1862C15.5988 21.5706 15.5362 21.9557 15.4735 22.3407C15.3333 23.1996 14.8134 23.7596 13.9685 23.9621C13.9392 23.9691 13.9126 23.9871 13.8848 24ZM13.6057 22.1179C13.6693 21.738 13.7362 21.3697 13.7919 20.9997C14.0895 19.0211 15.8739 17.7532 17.7824 18.1976C18.2878 18.3153 18.7709 18.5314 19.2601 18.7134C19.3805 18.7583 19.4418 18.7432 19.5019 18.6305C19.6156 18.4172 19.7413 18.2102 19.8622 18.0007C20.3687 17.1226 20.8752 16.2444 21.3878 15.3558C21.0733 15.0962 20.7675 14.8426 20.4604 14.5904C18.7663 13.1989 18.7667 10.7995 20.4612 9.40895C20.7683 9.15698 21.0741 8.90332 21.3887 8.64348C20.7505 7.5384 20.1202 6.44351 19.4817 5.35337C19.4544 5.30693 19.3393 5.27017 19.285 5.2883C18.9596 5.39663 18.6389 5.51908 18.319 5.64309C17.7161 5.87694 17.0975 5.96451 16.4589 5.83756C15.02 5.55133 14.0145 4.47601 13.7916 2.99857C13.7352 2.62458 13.6665 2.25229 13.6054 1.88993C12.2717 1.88993 10.9711 1.88993 9.66129 1.88993C9.59803 2.27204 9.53296 2.64059 9.47645 3.01045C9.17479 4.98551 7.37999 6.25061 5.47563 5.80061C4.99319 5.6866 4.53132 5.48107 4.06582 5.30236C3.90355 5.24004 3.82592 5.26361 3.73747 5.41963C3.15627 6.44445 2.56332 7.46258 1.97481 8.48321C1.94474 8.5354 1.92011 8.59072 1.89005 8.65035C2.18702 8.89557 2.47525 9.1346 2.76447 9.37232C4.52138 10.8163 4.52044 13.1836 2.76259 14.6292C2.46949 14.8701 2.17789 15.1131 1.88279 15.3575C2.52231 16.4657 3.15121 17.5527 3.77585 18.6423C3.83542 18.7461 3.89224 18.7588 4.00081 18.7159C4.35611 18.5756 4.71541 18.4453 5.07445 18.3148C7.01007 17.6107 9.06021 18.781 9.43901 20.8076C9.52027 21.2423 9.59078 21.679 9.66673 22.1179C10.983 22.1179 12.2782 22.1179 13.6057 22.1179Z"
                    fill="#7D889E"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M11.481 16.6241C8.88345 16.4903 6.88713 14.3363 7.00709 11.8292C7.13048 9.25013 9.27169 7.27275 11.7979 7.38227C14.3878 7.49447 16.3744 9.65943 16.2606 12.1584C16.1438 14.724 14.0277 16.6973 11.481 16.6241ZM14.3868 12.0065C14.3924 10.5077 13.1587 9.26245 11.6524 9.24663C10.1487 9.23082 8.88639 10.4817 8.88108 11.9929C8.87576 13.491 10.1103 14.7373 11.6154 14.7533C13.1183 14.7691 14.381 13.5177 14.3868 12.0065Z"
                    fill="#7D889E"
                  ></path>
                </svg>
              </Link>
            </div>
            <div className="grey-btn d-flex align-items-center justify-content-center ms-2">
              <Link
                to={"/dashboard"}
                className="d-flex align-items-center justify-content-center"
              >
                <svg
                  width="10.4"
                  height="12.3"
                  viewBox="0 0 16 19"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M6.46752 1C6.88931 1.14181 7.06155 1.44059 7.03211 1.87663C7.01955 2.06284 7.03005 2.25055 7.03005 2.43151C7.34382 2.51735 7.65253 2.58354 7.94886 2.6857C9.894 3.35647 11.2284 5.20174 11.2456 7.257C11.2575 8.68612 11.2517 10.1154 11.2464 11.5446C11.2452 11.8776 11.3521 12.1135 11.6555 12.2939C12.4644 12.7747 12.8283 13.744 12.5745 14.6534C12.3265 15.5422 11.4997 16.1839 10.5705 16.1855C7.74137 16.1901 4.9123 16.1901 2.08318 16.1855C1.14936 16.184 0.311924 15.5284 0.0745194 14.6363C-0.172027 13.7099 0.205223 12.7565 1.02812 12.2751C1.29952 12.1163 1.40636 11.8964 1.40594 11.597C1.40401 10.1796 1.40204 8.76202 1.40589 7.34453C1.41133 5.31135 2.61181 3.53011 4.49117 2.76385C4.66931 2.69118 4.85449 2.6337 5.03986 2.58148C5.22476 2.52949 5.41406 2.49297 5.6239 2.44449C5.6239 2.2588 5.63445 2.06687 5.62179 1.87653C5.59268 1.44063 5.76398 1.14135 6.18643 1.00005C6.28 1 6.37376 1 6.46752 1ZM6.30907 14.7812C7.70921 14.7812 9.1094 14.782 10.5096 14.7807C10.8646 14.7803 11.1287 14.5855 11.22 14.2655C11.3087 13.9547 11.189 13.6439 10.8893 13.4704C10.1577 13.0465 9.84238 12.3987 9.84248 11.5736C9.84262 10.1852 9.84599 8.79671 9.83943 7.40829C9.83821 7.15213 9.81622 6.8925 9.77023 6.64061C9.43095 4.78394 7.54648 3.50583 5.69853 3.87136C3.98068 4.21115 2.81419 5.62414 2.81166 7.37266C2.8096 8.8021 2.81982 10.2317 2.80763 11.661C2.80097 12.4356 2.47913 13.0497 1.79434 13.4517C1.70532 13.504 1.61756 13.5724 1.55506 13.6533C1.38357 13.8755 1.35198 14.125 1.47659 14.3798C1.60696 14.6464 1.82997 14.7801 2.12607 14.7804C3.5204 14.782 4.91473 14.7811 6.30907 14.7812Z"
                    fill="#7D889E"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M4.77914 18.9999C4.73127 18.9792 4.68317 18.959 4.63564 18.9376C4.3394 18.8041 4.16823 18.4859 4.22599 18.1767C4.28684 17.8507 4.5557 17.5996 4.88485 17.5972C5.84549 17.59 6.80622 17.59 7.76681 17.5972C8.09587 17.5996 8.36435 17.8503 8.42562 18.1766C8.48366 18.4856 8.31274 18.8038 8.01631 18.9375C7.96877 18.959 7.92072 18.9792 7.8729 18.9999C6.84166 18.9999 5.81042 18.9999 4.77914 18.9999Z"
                    fill="#7D889E"
                  ></path>
                  <circle
                    cx="12"
                    cy="4"
                    r="4"
                    fill="url(#paint0_linear_100_284)"
                  ></circle>
                  <defs>
                    <linearGradient
                      id="paint0_linear_100_284"
                      x1="8.6506"
                      y1="0.622222"
                      x2="16.0861"
                      y2="1.03621"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stopColor="#3B5998"></stop>
                      <stop offset="1" stopColor="#6BF3AA"></stop>
                      <stop offset="1" stopColor="#3EFE97"></stop>
                    </linearGradient>
                  </defs>
                </svg>
              </Link>
            </div>
            <div className="position-relative ms-3">
              <Link to={"/dashboard"} className="position-relative">
                <img src={avatar} alt="" />
              </Link>
              <img
                src={badge}
                className="position-absolute avatar-badge"
                width="18"
                height="18"
                alt=""
              />
            </div>
          </div>
        </header>

        <nav className="nav-drill">
          <ul className="nav-items nav-level-1">
            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_96_408)">
                    <path
                      d="M15.8333 4.16667V5.83333H12.5V4.16667H15.8333ZM7.5 4.16667V9.16667H4.16667V4.16667H7.5ZM15.8333 10.8333V15.8333H12.5V10.8333H15.8333ZM7.5 14.1667V15.8333H4.16667V14.1667H7.5ZM17.5 2.5H10.8333V7.5H17.5V2.5ZM9.16667 2.5H2.5V10.8333H9.16667V2.5ZM17.5 9.16667H10.8333V17.5H17.5V9.16667ZM9.16667 12.5H2.5V17.5H9.16667V12.5Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_96_408">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Dashboard</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_99_194)">
                    <path
                      d="M15 9.16675C15 9.72508 15 10.2751 15 10.8334C16 10.8334 17.3 10.8334 18.3333 10.8334C18.3333 10.2751 18.3333 9.72508 18.3333 9.16675C17.3 9.16675 16 9.16675 15 9.16675Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M13.333 14.6749C14.133 15.2666 15.1747 16.0499 15.9997 16.6666C16.333 16.2249 16.6663 15.7749 16.9997 15.3333C16.1747 14.7166 15.133 13.9333 14.333 13.3333C13.9997 13.7833 13.6663 14.2333 13.333 14.6749Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M16.9997 4.66659C16.6663 4.22492 16.333 3.77492 15.9997 3.33325C15.1747 3.94992 14.133 4.73325 13.333 5.33325C13.6663 5.77492 13.9997 6.22492 14.333 6.66659C15.133 6.06659 16.1747 5.29159 16.9997 4.66659Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M3.33366 7.5C2.41699 7.5 1.66699 8.25 1.66699 9.16667V10.8333C1.66699 11.75 2.41699 12.5 3.33366 12.5H4.16699V15.8333H5.83366V12.5H6.66699L10.8337 15V5L6.66699 7.5H3.33366ZM7.52533 8.925L9.16699 7.94167V12.0583L7.52533 11.075L7.12533 10.8333H6.66699H3.33366V9.16667H6.66699H7.12533L7.52533 8.925Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M12.917 9.99992C12.917 8.89159 12.4337 7.89159 11.667 7.20825V12.7833C12.4337 12.1083 12.917 11.1083 12.917 9.99992Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_99_194">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Campaign</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_100_202)">
                    <path
                      d="M19.1667 9.17508L15 9.16675C14.5417 9.16675 14.1667 9.54175 14.1667 10.0001V17.5001C14.1667 17.9584 14.5417 18.3334 15 18.3334H19.1667C19.625 18.3334 20 17.9584 20 17.5001V10.0001C20 9.54175 19.625 9.17508 19.1667 9.17508ZM19.1667 16.6667H15V10.8334H19.1667V16.6667ZM16.6667 1.66675H1.66667C0.741667 1.66675 0 2.40841 0 3.33341V13.3334C0 14.2501 0.741667 15.0001 1.66667 15.0001H7.5V16.6667H5.83333V18.3334H12.5V16.6667H10.8333V15.0001H12.5V13.3334H1.66667V3.33341H16.6667V7.50008H18.3333V3.33341C18.3333 2.40841 17.5833 1.66675 16.6667 1.66675ZM9.975 7.50008L9.16667 5.00008L8.35833 7.50008H5.83333L7.89167 8.96675L7.10833 11.3917L9.16667 9.89175L11.225 11.3917L10.4417 8.96675L12.5 7.50008H9.975Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_100_202">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Ads Manager</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_194_302)">
                    <path
                      d="M3.33366 5.00008H16.667V6.66675H3.33366V5.00008ZM5.00033 1.66675H15.0003V3.33341H5.00033V1.66675ZM16.667 8.33341H3.33366C2.41699 8.33341 1.66699 9.08341 1.66699 10.0001V16.6667C1.66699 17.5834 2.41699 18.3334 3.33366 18.3334H16.667C17.5837 18.3334 18.3337 17.5834 18.3337 16.6667V10.0001C18.3337 9.08341 17.5837 8.33341 16.667 8.33341ZM16.667 16.6667H3.33366V10.0001H16.667V16.6667ZM8.33366 10.6084V16.0501L13.3337 13.3334L8.33366 10.6084Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_194_302">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Ad Sets</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_194_305)">
                    <path
                      d="M17.842 9.65008L10.342 2.15008C10.042 1.85008 9.62533 1.66675 9.16699 1.66675H3.33366C2.41699 1.66675 1.66699 2.41675 1.66699 3.33341V9.16675C1.66699 9.62508 1.85033 10.0417 2.15866 10.3501L9.65866 17.8501C9.95866 18.1501 10.3753 18.3334 10.8337 18.3334C11.292 18.3334 11.7087 18.1501 12.0087 17.8417L17.842 12.0084C18.1503 11.7084 18.3337 11.2917 18.3337 10.8334C18.3337 10.3751 18.142 9.95008 17.842 9.65008ZM10.8337 16.6751L3.33366 9.16675V3.33341H9.16699V3.32508L16.667 10.8251L10.8337 16.6751Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M5.41699 6.66675C6.10735 6.66675 6.66699 6.1071 6.66699 5.41675C6.66699 4.72639 6.10735 4.16675 5.41699 4.16675C4.72664 4.16675 4.16699 4.72639 4.16699 5.41675C4.16699 6.1071 4.72664 6.66675 5.41699 6.66675Z"
                      fill="#7D889E"
                    ></path>
                    <path
                      d="M7.41699 10.4583C7.41699 10.9333 7.60866 11.35 7.91699 11.6667L10.8337 14.5833L13.7503 11.6667C14.0587 11.3583 14.2503 10.925 14.2503 10.4583C14.2503 9.51667 13.4837 8.75 12.542 8.75C12.067 8.75 11.642 8.94167 11.3337 9.25L10.8337 9.75L10.3337 9.25833C10.0253 8.94167 9.59199 8.75 9.12533 8.75C8.18366 8.75 7.41699 9.51667 7.41699 10.4583Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_194_305">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Ads</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_195_310)">
                    <path
                      d="M12.3503 9.16658H18.292C17.9003 5.23325 14.767 2.09992 10.8337 1.70825V7.64992C11.542 7.90825 12.092 8.45825 12.3503 9.16658ZM12.5003 3.81659C14.167 4.49992 15.5003 5.83325 16.1837 7.49992H13.3253C13.092 7.19159 12.8087 6.90825 12.5003 6.67492V3.81659ZM1.66699 9.99992C1.66699 14.3249 4.95866 17.8749 9.16699 18.2916V12.3499C8.20033 11.9999 7.50033 11.0833 7.50033 9.99992C7.50033 8.91658 8.20033 7.99992 9.16699 7.64992V1.70825C4.95866 2.12492 1.66699 5.67492 1.66699 9.99992ZM7.50033 3.81659V6.68325C6.47533 7.44992 5.83366 8.67492 5.83366 9.99992C5.83366 11.3249 6.47533 12.5499 7.50033 13.3249V16.1916C5.03366 15.1999 3.33366 12.7916 3.33366 9.99992C3.33366 7.20825 5.03366 4.79992 7.50033 3.81659ZM10.8337 12.3499V18.2916C14.767 17.8999 17.9003 14.7666 18.292 10.8333H12.3503C12.092 11.5416 11.542 12.0916 10.8337 12.3499ZM12.5003 13.3249C12.8087 13.0916 13.092 12.8166 13.3253 12.4999H16.1837C15.5003 14.1666 14.167 15.4999 12.5003 16.1833V13.3249Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_195_310">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Analytics</span>
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link" to={"/dashboard"}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_193_296)">
                    <path
                      d="M13.3337 9.16667V2.5H6.66699V7.5H1.66699V17.5H18.3337V9.16667H13.3337ZM8.33366 4.16667H11.667V15.8333H8.33366V4.16667ZM3.33366 9.16667H6.66699V15.8333H3.33366V9.16667ZM16.667 15.8333H13.3337V10.8333H16.667V15.8333Z"
                      fill="#7D889E"
                    ></path>
                  </g>
                  <defs>
                    <clipPath id="clip0_193_296">
                      <rect width="20" height="20" fill="white"></rect>
                    </clipPath>
                  </defs>
                </svg>
                <span className="ms-3">Leads</span>
              </Link>
            </li>

            <li className="nav-item nav-expand">
              <Link className="nav-link nav-expand-link" to={"/dashboard"}>
                More
                <svg
                  className="mx-1"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M16.59 8.59009L12 13.1701L7.41 8.59009L6 10.0001L12 16.0001L18 10.0001L16.59 8.59009Z"
                    fill="white"
                  ></path>
                </svg>
              </Link>
              <ul className="nav-items nav-expand-content">
                <li className="nav-item">
                  <Link className="nav-link nav-back-link" to={"/dashboard"}>
                    <svg
                      className="mx-1"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M16.59 8.59009L12 13.1701L7.41 8.59009L6 10.0001L12 16.0001L18 10.0001L16.59 8.59009Z"
                        fill="white"
                      ></path>
                    </svg>{" "}
                    Back
                  </Link>
                </li>
                <li className="nav-item">
                  <Link className="nav-link" to={"/dashboard"}>
                    <svg
                      width="18"
                      height="18"
                      viewBox="0 0 18 18"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M7.84059 0C8.61395 0 9.38732 0 10.1607 0C10.4239 0.0922761 10.5561 0.286496 10.6076 0.553657C10.6484 0.765892 10.7003 0.97637 10.7609 1.18377C10.7767 1.23826 10.8352 1.30768 10.8866 1.32087C11.6692 1.51728 12.4039 1.82575 13.0977 2.23704C13.1443 2.26472 13.2339 2.26121 13.2831 2.2344C13.4888 2.12367 13.6856 1.99624 13.8878 1.87892C14.1839 1.70711 14.4045 1.73347 14.6479 1.97471C15.1093 2.43214 15.5685 2.89176 16.0264 3.3527C16.2689 3.59701 16.2957 3.81628 16.1239 4.11244C16.0066 4.31457 15.8792 4.51143 15.7684 4.71707C15.7416 4.76672 15.7372 4.85592 15.7645 4.90206C16.1758 5.59589 16.4842 6.33058 16.6811 7.11317C16.6938 7.16458 16.7624 7.2239 16.8164 7.23972C17.0238 7.30036 17.2343 7.35265 17.4461 7.39352C17.7133 7.44493 17.9088 7.57543 18.0006 7.83952C18.0006 8.61288 18.0006 9.38624 18.0006 10.1596C17.9097 10.4145 17.7251 10.5489 17.4628 10.5999C17.2448 10.6421 17.0291 10.6966 16.8164 10.7594C16.7624 10.7752 16.6938 10.8345 16.6811 10.8864C16.4842 11.669 16.1758 12.4037 15.7645 13.0971C15.7372 13.1432 15.7416 13.2328 15.7684 13.2821C15.8822 13.4925 16.0136 13.6942 16.1323 13.9021C16.2927 14.1828 16.2663 14.403 16.0387 14.6337C15.5738 15.1038 15.1062 15.5718 14.6356 16.0367C14.4063 16.2635 14.183 16.2889 13.9031 16.129C13.7001 16.013 13.5046 15.8834 13.2976 15.7753C13.2401 15.7449 13.1372 15.741 13.0832 15.7726C12.3977 16.1743 11.6749 16.4796 10.9037 16.6747C10.8444 16.6897 10.7776 16.7701 10.7583 16.8334C10.6963 17.0342 10.6475 17.2394 10.6076 17.4459C10.5561 17.7131 10.4239 17.9068 10.1607 17.9996C9.38732 17.9996 8.61395 17.9996 7.84059 17.9996C7.57826 17.9051 7.44644 17.7109 7.39547 17.4433C7.35636 17.2367 7.30671 17.0311 7.24431 16.8303C7.22498 16.7675 7.15775 16.6958 7.09623 16.6743C6.88751 16.5996 6.66737 16.5565 6.45909 16.4814C6.17611 16.3795 5.89972 16.2591 5.6119 16.1431C5.64135 16.1035 5.65013 16.0877 5.662 16.0754C6.44063 15.2963 7.21795 14.5164 8.00142 13.7426C8.04887 13.6955 8.14906 13.6692 8.21717 13.6802C8.69041 13.7571 9.16146 13.7408 9.63427 13.6788C10.8031 13.5259 11.7909 13.0215 12.5621 12.1317C13.5213 11.0244 13.8987 9.72854 13.6764 8.28113C13.4321 6.69222 12.5818 5.50493 11.1515 4.77815C9.70896 4.04521 8.23475 4.06323 6.79963 4.80803C5.58027 5.44078 4.7924 6.44571 4.42506 7.7758C4.24402 8.43096 4.23391 9.09315 4.33014 9.76018C4.34684 9.87531 4.3174 9.95484 4.23391 10.0379C3.49087 10.7774 2.75222 11.5222 2.01225 12.2653C1.97183 12.3061 1.92877 12.3444 1.87868 12.3918C1.85099 12.3386 1.82463 12.2995 1.80925 12.2565C1.65238 11.8228 1.48936 11.3908 1.34743 10.9523C1.30568 10.8236 1.2534 10.7581 1.12113 10.7344C0.937022 10.7014 0.757739 10.6438 0.574066 10.6096C0.302511 10.5586 0.0986234 10.4294 0.00195312 10.1596C0.00195312 9.38624 0.00195312 8.61288 0.00195312 7.83952C0.0986234 7.57016 0.30207 7.44141 0.573186 7.38956C0.779709 7.35002 0.984477 7.29948 1.18617 7.24016C1.24065 7.2239 1.31052 7.1659 1.32326 7.11493C1.51968 6.33234 1.82814 5.59765 2.23943 4.90382C2.26711 4.85724 2.2636 4.7676 2.23723 4.71839C2.1265 4.51274 1.99951 4.31589 1.88175 4.11376C1.70994 3.81848 1.73631 3.59657 1.97754 3.35358C2.43497 2.8922 2.89459 2.43302 3.35553 1.97559C3.59941 1.73347 3.81999 1.70667 4.11527 1.87804C4.3174 1.99536 4.51426 2.12279 4.7199 2.23352C4.76955 2.26033 4.8592 2.26472 4.90533 2.23748C5.59916 1.82619 6.33342 1.51772 7.116 1.32087C7.16742 1.30812 7.22674 1.23958 7.24256 1.18509C7.30363 0.978127 7.35636 0.76765 7.39679 0.555415C7.44644 0.287814 7.5787 0.0944732 7.84059 0Z"
                        fill="#7D889E"
                      ></path>
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M1.75764 18C1.64032 18 1.52344 18 1.40611 18C1.37931 17.9903 1.35338 17.9767 1.3257 17.9718C0.931546 17.9007 0.606825 17.7148 0.350648 17.4028C0.155111 17.1651 0.0694268 16.8839 0 16.5934C0 16.4761 0 16.3592 0 16.2419C0.0742603 15.733 0.380965 15.3701 0.732053 15.0216C2.29504 13.4701 3.84835 11.9093 5.40826 10.3547C5.4935 10.2699 5.5124 10.1991 5.4834 10.0809C5.41529 9.80365 5.33531 9.52199 5.32301 9.23944C5.26237 7.86102 5.78308 6.74535 6.92071 5.95441C7.84215 5.31331 8.86905 5.16875 9.95439 5.43196C10.1978 5.49084 10.3762 5.69604 10.3464 5.92585C10.327 6.07613 10.2488 6.24223 10.146 6.3534C9.74831 6.7827 9.31505 7.17949 8.92398 7.61407C8.60673 7.96691 8.62342 8.5298 8.91563 8.89583C9.32736 9.41213 10.0273 9.4486 10.5195 8.97053C10.9101 8.59088 11.2867 8.19716 11.6778 7.81839C12.0025 7.50377 12.4762 7.64087 12.5777 8.07545C12.699 8.59351 12.7258 9.11509 12.6295 9.64282C12.1862 12.0684 9.91924 13.123 7.92124 12.5232C7.80568 12.4884 7.73405 12.5043 7.64749 12.5908C6.09769 14.1468 4.54043 15.6957 2.9937 17.2552C2.63998 17.6115 2.26867 17.9156 1.75764 18Z"
                        fill="#7D889E"
                      ></path>
                    </svg>

                    <span className="ms-3">Setting</span>
                  </Link>
                </li>
                <li className="nav-item">
                  <Link className="nav-link" to={"/dashboard"}>
                    <svg
                      width="18"
                      height="18"
                      viewBox="0 0 18 18"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9 0C4.032 0 0 4.032 0 9C0 13.968 4.032 18 9 18C13.968 18 18 13.968 18 9C18 4.032 13.968 0 9 0ZM15.714 6.408L13.212 7.443C12.753 6.219 11.79 5.247 10.557 4.797L11.592 2.295C13.482 3.015 14.985 4.518 15.714 6.408ZM9 11.7C7.506 11.7 6.3 10.494 6.3 9C6.3 7.506 7.506 6.3 9 6.3C10.494 6.3 11.7 7.506 11.7 9C11.7 10.494 10.494 11.7 9 11.7ZM6.417 2.286L7.47 4.788C6.228 5.238 5.247 6.219 4.788 7.461L2.286 6.417C3.015 4.518 4.518 3.015 6.417 2.286ZM2.286 11.583L4.788 10.548C5.247 11.79 6.219 12.762 7.461 13.212L6.408 15.714C4.518 14.985 3.015 13.482 2.286 11.583ZM11.592 15.714L10.557 13.212C11.79 12.753 12.762 11.781 13.212 10.539L15.714 11.592C14.985 13.482 13.482 14.985 11.592 15.714Z"
                        fill="#7D889E"
                      ></path>
                    </svg>
                    <span className="ms-3">Support</span>
                  </Link>
                </li>
              </ul>
            </li>
          </ul>
        </nav>
      </div>
    </div>
  );
}
